...
def main(args):
    ...
    # betas = [0.01, 0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0] # for rib
    # betas = [0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0] # for vib
    betas = [0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0] # for nib & dib
    for seed, s_seed, beta, config, ns in itertools.product(seeds, s_seeds, betas,
                                                            configs,
                                                            num_samples):
        task_name = f"{dataset}_{config['method']}_SEED{seed}_SSEED{s_seed}_NS{ns}_BETA{beta}"
...
if __name__ == '__main__':
    ...
    base_cfg = {'warm': 0, 'epoch': 100, 'bs': 128, 'lr': 1e-3,
                'wd': 0.0, 'dropout': 0.0, 'es': 20, 'nh': 3, 'lam': 10.0}
    ...
    configs = [nib_cfg, dib_cfg]
    # configs = [vanilla_cfg, l2_cfg, dropout_cfg, pib_cfg, vib_cfg, nib_cfg, dib_cfg, rib_cfg, rib_minimax_cfg]
